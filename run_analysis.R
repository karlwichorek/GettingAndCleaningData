...
# Get the data
# Download the file and save it to your working directory.
...
parameters <- "x"
cmd <- paste(paste0("\"", executable, "\""), parameters, 
             paste0("\"", file.path(path, f), "\""))
system(cmd)
...
# Read the data files. Getting some errors with data.table's fread function
# using read.table instead.
...
# This tells you which variables in `dt` are measurements 
...
# Label with descriptive activity names and merge the activity labels.
dt <- merge(dt, dtActivityNames, by="activityNum", all.x=TRUE)

# Add `activityName` as a key.
setkey(dt, subject, activityNum, activityName)

# Melt the data table to reshape it from a short and wide format
# to a tall and narrow format.
...
# Create a new variable, `activity` that is equivalent to `activityName` 
# as a factor class.
...
# Make codebook and dataset.
knit("GenerateCodebook.Rmd", output="codebook.md", 
